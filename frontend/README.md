...
- **Dev:** Vite proxies `/api/*` to the PHP server (`VITE_PHP_ORIGIN`, default
  `http://localhost:8000`). Run PHP with `php -S localhost:8000 -t public`.
- **Prod:** `npm run build` outputs to `../public/app/`. Serve that from the same
  domain as PHP. Add PHP routes under `/api/...` that return JSON
  (`{ "data": ... }` / `{ "error": ... }`), reusing the existing controllers
  (`Projects`, `OpenSource`, `AuthControler`, …).
